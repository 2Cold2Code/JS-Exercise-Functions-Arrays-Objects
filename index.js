 ...
 */
function getLastCarInfo( /* code here */ list) {
    /* code here */
    let last = list.length - 1;
    return `This is a ${list[last].car_make} ${list[last].car_model}`;
}
...
function getCarInfoById( /* code here */ list, idIn) {
    /* code here */
    let idOutput;
    ...
    function findIt(list) {
        for (let i = 0; i < list.length; i++) {
            if (list[i].id === idIn) {
                return `This is a ${list[i].car_make} ${list[i].car_model}`;
            }
        }
    }
    idOutput = findIt(list);
    return idOutput;
}
/*   function getObject(arr) {              
        //make a function that takes an array as an argument and iterates through that array returning each index
        for (let iArr = 0; iArr < list.length; iArr++) {
            return list[iArr];
        }

        function makeNames(arr) {
            //make a function that takes an array as an argument and creates a list of names called (array's name)Obj(index number of anonymous object within the array)
            const objNamesList = [];
            for (let i = 0; i < arr.length; i++) {
                objNamesList.push(`${arr}Obj${i+1}`);
                return objNamesList;
            }
        }

        function nameArrObj(arr, make, get) {
            //make a function that takes an array (that holds anonymous objects at its indices), a function that creates constant variables, and a function that retrieves objects from the array, and returns 'const (variable name) = (object properties);
            for (let name = 0; name < arr.length; name++) {
                return `const ${make(arr[name])} = ${get(arr[name])}`;
            }
        }

        const arrOfNamedObjs = nameArrObj(list, makeNames, getObject);
        //call arrOfNamedObjs and return constant variables as defined by namedArrObj function

        return arrOfNamedObjs.car_model;
*/
/*This way works as well, but I don't believe it's how we're meant to do it*/
// return `This is a ${list[id - 1].car_make} ${list[id - 1].car_model}`

//how not to do it
// return `This is a ${list['id'].car_make} ${list['id'].car_model}`;
 ...
 */
function carMaker( /* code here */ odom) {
    /* code here */
    const Car = {
        odometer: odom,
